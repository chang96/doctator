    ...
    const r = async () => {
      ...
      if (p) {
        ...
        if (!o.defaultProject.set) {
          ...
          if (selected) {
            if (selected === "paths")
              setState({ fieldName: "paths", value: stringifire(p) });
            if (selected === "config")
              setState({ fieldName: "config", value: stringifire(c) });
          }
        } else {
          if (selected === "paths")
            setState({
              fieldName: "paths",
              value: stringifire(o.defaultProject.paths),
            }); //JSON.stringify(state.value, null, "   ")
          if (selected === "config")
            setState({
              fieldName: "config",
              value: stringifire(o.defaultProject.config),
            });
        }
      }
    };
